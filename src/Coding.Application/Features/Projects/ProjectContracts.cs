...
public sealed record CreateProjectCommand(string Name, string? Description, string DefaultLanguage, bool IsPublic) : IRequest<ProjectDetails>;
...
public sealed class InviteProjectMemberValidator : AbstractValidator<InviteProjectMemberCommand>
{
    public InviteProjectMemberValidator()
    {
        ...
        RuleFor(command => command.Role).Must(role => role is ProjectRole.Admin or ProjectRole.Member)
            .WithMessage("Invitations may grant only Admin or Member roles.");
    }
}
...
public sealed class ChangeProjectMemberRoleValidator : AbstractValidator<ChangeProjectMemberRoleCommand>
{
    public ChangeProjectMemberRoleValidator() => RuleFor(command => command.Role)
        .Must(role => role is ProjectRole.Admin or ProjectRole.Member)
