...
public sealed record AdminUserListItem(Guid Id, string DisplayName, string UserName, string Email, bool IsSuspended, DateTime? BanExpiresAt, IReadOnlyList<string> Roles, DateTime CreatedAt, DateTime LastSeen);
public sealed record AdminUserDetails(Guid Id, string FirstName, string LastName, string UserName, string Email, string? Bio, string? AvatarUrl, bool IsSuspended, string? SuspensionReason, DateTime? BanExpiresAt, IReadOnlyList<string> Roles, int ProjectCount, DateTime CreatedAt, DateTime LastSeen);
...
public sealed record SetUserSuspensionCommand(Guid UserId, bool Suspended, string? Reason, DateTime? ExpiresAt = null) : IRequest;
